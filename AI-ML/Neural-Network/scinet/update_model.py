...
class Evolution(keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.weights = self.add_weights(shape=(self.input_dim,), initializer='random_normal')
        self.biases = self.add_weights(shape=(self.input_dim,), initializer='random_normal')

    def call(self, inputs):
        return inputs[:,:-1] + inputs[:,-1].reshape(-1,1)*self.weights + self.biases
"""
...
class Evolution(keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.update_step1 = self.add_weights(shape=(self.input_dim, 1), initializer='random_normal')
        self.update_step2 = self.add_weights(shape=(self.input_dim, 1), initializer='random_normal')
        self.output = np.zeros(self.input_dim*self.units).reshape(self.input_dim, self.units)

    def call(self, inputs):
        self.output[i:i+1,:] = inputs
        for i in range(1, self.units):
            self.output[i:i+1,:] = self.output[i-1:i,:] + self.weights
        return self.output
        #return inputs[:,:-1] + inputs[:,:-1] + self.weights
...
class Decoder(keras.layers.Layer):
    def __init__(self, layer_size=[70, 70], output_size=1, activation='elu', **kwargs):
        ...
        self.decoder_output = Dense(1, name='decoder_layer_out')
...
class ScNN(Model):
    #
    def __init__(self, encoder_layer=[70, 70], latent_size=3, decoder_layer=[70, 70], output_size=1, beta_rec=500., beta_kl=1., target_var=0.01, activation='elu', **kwargs):
        super().__init__(**kwargs)
        ...
        self.evolve = Evolution(units=output_size, name='evolution_layer')
    ...
    def call(self, inputs):
        mean, log_var, decoder_input = self.encoder(inputs)

        samples = decoder_input.shape[0]
        decoder_input = self.evolve(decoder_input)
        decoder_input = decoder_input.reshape(-1,1)
        predictions = self.decoder(decoder_input)
        predictions = predictions.reshape(samples, self.latent_size*self.output_size)
        
        return mean, log_var, decoder_input, predictions
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            mean, log_var, z, predictions = self(inputs)
