...
class Matrix
     {public:
      ...
      Matrix(float xmax, float tmax, float x_step)    
         {xrange = xmax;
          trange = tmax;
          dx = x_step;
          u=NULL;
...
class MatrixOperation
     {public:
      ...
      //Temporal boundary conditions as inputs
       //At a given t_index boundary value is constant
      static void tBoundary(float boundary, int t_index, float **u)   
          {for(int j=0; j<Nx; ++j)
              {u[t_index][j] = boundary;
               }
           }

      //At a given t_index boundary value depends on x
      static void tBoundary(float (*boundary)(float x_value), int t_index, float **u)    
          {for(int j=0; j<Nx; ++j)
              {u[t_index][j] = boundary(j*dx);
               }
           }

      //At a given t_index boundary value depends on x and t
      static void tBoundary(float (*boundary)(float x_value, int t_value), int t_index, float **u)  
          {for(int j=0; j<Nx; ++j)
              {u[t_index][j] = boundary(j*dx, t_index*dt);
               }
           }

      //At a given t_index boundary value is constant. Boundary is set for given range of x
      static void tBoundary(float boundary, int xstart_index, int xlast_index, int t_index, float **u)      
          {for(int j=xstart_index; j<=xlast_index; ++j)
              {u[t_index][j] = boundary;
               }
           }

      //At a given t_index boundary value depends on x. Boundary is set for given range of x
      static void tBoundary(float (*boundary)(float x_value), int xstart_index, int xlast_index, int t_index, float **u)      
          {for(int j=xstart_index; j<=xlast_index; ++j)
              {u[t_index][j] = boundary(j*dx);
               }
           }

      //At a given t_index boundary value depends on x and t. Boundary is set for given range of x
      static void tBoundary(float (*boundary)(float x_value, int t_value), int xstart_index, int xlast_index, int t_index, float **u)   
          {for(int j=xstart_index; j<=xlast_index; ++j)
              {u[t_index][j] = boundary(j*dx, t_index*dt);
               }
           }
      ...
      //At a given x_index boundary value is constant
      static void xBoundary(float boundary, int x_index, float **u)      
          {for(int j=0; j<Nt; ++j)
              {u[j][x_index] = boundary;
               }
           }

      //At a given x_index boundary value depends on x and t
      static void xBoundary(float (*boundary)(float x_value, int t_value), int x_index, float **u)    
          {for(int j=0; j<Nt; ++j)
              {u[j][x_index] = boundary(x_index*dx, j*dt);
               }
           }

      //At a given x_index boundary value depends on t
      static void xBoundary(float (*boundary)(float t_value), int x_index, float **u)      
          {for(int j=0; j<Nt; ++j)
      ...
      //At a given x_index boundary value is constant. 
      //Boundary is set for given range of t
      static void xBoundary(float boundary, int tstart_index, int tlast_index, int x_index, float **u)      
      ...
      //At a given x_index boundary value depends on t. 
      //Boundary is set for given range of t
      static void xBoundary(float (*boundary)(int t_value), int tstart_index, int tlast_index, int x_index, float **u)      
          {for(int j=tstart_index; j<=tlast_index; ++j)
              {u[j][x_index] = boundary(j*dt);
               }
           }
      ...
      //At a given x_index boundary value depends on x and t. 
      //Boundary is set for given range of t
      static void xBoundary(float (*boundary)(float x_value, int t_value), int tstart_index, int tlast_index, int x_index, float **u)     
          {for(int j=tstart_index; j<=tlast_index; ++j)
              {u[j][x_index] = boundary(x_index*dx, j*dt);
               }
           }
      ...
      static void setMatrix(float (*function)(float **matrix, int x_index, int t_index, int row, int col), int xstart_index, int xlast_index, int tstart_index, int tlast_index, float **u)               //function
       ...
       static void fileOutput(string file_name, float **u)  //writes function values to a file 
           {fstream foutn;
            foutn.open(file_name,ios::out|ios::trunc);
